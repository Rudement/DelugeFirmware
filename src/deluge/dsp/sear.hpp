...
inline void searBuffer(q31_t* startSample, q31_t* endSample, q31_t level, SearLevel& state) {
	...
	const int32_t octaves = level >> 26;
	...
	do {
		...
		*currentSample = y;
		...
		const q31_t a = (c < 0) ? subtract_saturate(0, c) : c;
		const q31_t b = (y < 0) ? subtract_saturate(0, y) : y;
	...
	const q31_t target = add_saturate(raw, 2 * multiply_32x32_rshift32(raw, lift));
