 ...
 * in the automation lists, the MIDI-follow CC maps and the two shortcut grids. The one thing
 * it buys, and the reason to pay: a bool cannot be recorded into a clip, thrown by a gold knob
 * or driven by a CC. For a bypass that is the difference between a setup option and a
 * performance gesture.
 *
...
public:
	...
	void getNotificationValue(StringBuf& valueBuf) override {
		valueBuf.append(l10n::get(getValue() != 0 ? l10n::String::STRING_FOR_ON : l10n::String::STRING_FOR_OFF));
	}
...
protected:
	...
	// One slot of a horizontal menu: the same switcher icon Toggle uses.
	void renderInHorizontalMenu(const SlotPosition& slot) override {
		using namespace deluge::hid::display;
		const Icon& icon = getValue() != 0 ? OLED::switcherIconOn : OLED::switcherIconOff;
